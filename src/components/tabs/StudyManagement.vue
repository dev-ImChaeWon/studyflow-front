<template>
    ...
    <div class="filter-header">
        ...
        <div class="tag-container">
            <div class="tags">
                <SearchTag v-for="t in searchTags" :key="t.id" :label="t.type" :value="t.value" :id="t.id"
                    @click-tag="removeTag" />
            </div>
            <button @click="() => { studentName = ''; selectedTeacher = 'all' }" class="icon-btn"><svg width="20px"
                    height="20px" viewBox="0 0 24 24" stroke-width="1.5" fill="none" xmlns="http://www.w3.org/2000/svg"
                    color="#000000">
                    <path
    ...
    <div v-if="studentList.length > 0" class="list-box">
        <StudentCard v-for="s in studentList" v-bind:key="s.studentId" 
            :student="s"
        />
        <StudentPagination :totalItems="totalItems" :itemsPerPage="10" :currentPage="currentPage"
